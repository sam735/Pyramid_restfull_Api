from db.model.allergies import (FhirAllergy, CodeYn, FhirCodeableConcept,
                                Reference, FhirIdentifier, Annotation, AllergyReaction, AllergyCategory)
...
def insert_allergies(request):
    ...
    for key, val in allergies_payload.items():
        if type(allergies[key]).__name__ == 'list':
            for val in allergies[key]:
                if type(val).__name__ == 'Reference':
                    ref_obj = Reference(
                        val, allergies_obj.fhir_allergy_idn, 'allergy', key)
                    request.db.add(ref_obj)
                if type(val).__name__ == 'FhirCodeableConcept':
                    if val.coding != None:
                        for code in val.coding:
                            Codeeable_obj = FhirCodeableConcept(
                                (code), val.text, allergies_obj.fhir_allergy_idn, 'allergy', key)
                            request.db.add(Codeeable_obj)
                    else:
                        Codeeable_obj = FhirCodeableConcept(DictToObject(
                            {}), val.text, allergies_obj.fhir_allergy_idn, 'allergy', key)
                        request.db.add(Codeeable_obj)

                if type(val).__name__ == 'Annotation':
                    insert_to_annotation(
                        val, allergies_obj.fhir_allergy_idn, 'allergy', key, request.db)
        ...
        if type(allergies[key]).__name__ == 'Reference':
            ref_obj = Reference(
                allergies[key], allergies_obj.fhir_allergy_idn, 'allergy', key)
            request.db.add(ref_obj)
        ...
        if type(allergies[key]).__name__ == 'FhirCodeableConcept':
            if allergies[key].coding != None:
                for code in allergies[key].coding:
                    Codeeable_obj = FhirCodeableConcept(
                        (code), allergies[key].text, allergies_obj.fhir_allergy_idn, 'allergy', key)
                    request.db.add(Codeeable_obj)
            else:
                Codeeable_obj = FhirCodeableConcept(DictToObject(
                    {}), allergies[key].text, allergies_obj.fhir_allergy_idn, 'allergy', key)
                request.db.add(Codeeable_obj)
    ...
    if allergies.category != None:
        for category in allergies.category:
            category_obj = AllergyCategory(
                category, allergies_obj.fhir_allergy_idn)
    ...
    if allergies.reaction != None:
        for reaction in allergies.reaction:
            if reaction.get('description') or reaction.get('onset') or reaction.get('severity'):
                allergy_reaction_obj = AllergyReaction(((reaction.get('substance').coding[0].display if reaction.get('substance').coding else None) if reaction.get(
                    'substance') else None), reaction.get('onset'), reaction.get('severity'), allergies_obj.fhir_allergy_idn)
                ...
                for obj in reaction.get('manifestation'):
                    if obj.coding != None:
                        for code in obj.coding:
                            manifestation_obj = FhirCodeableConcept(
                                (code), code.display, allergies_obj.fhir_allergy_idn, 'allergy', 'manifestation')
            ...
            if reaction.get('exposureRoute') != None:
                if reaction.get('exposureRoute').coding != None:
                    for code in reaction.get('exposureRoute').coding:
                        exposure_obj = FhirCodeableConcept(
                            (code), code.display, allergies_obj.fhir_allergy_idn, 'allergy', 'exposureRoute')
            ...
            if reaction.get('note') != None:
                for obj in reaction.get('note'):
                    insert_to_annotation(
                        obj, allergies_obj.fhir_allergy_idn, 'allergy', key, request.db)
                    request.db.flush()

            if reaction.get('substance') != None:
                if reaction.get('substance').coding != None:
                    for code in reaction.get('substance').coding:
                        substance_obj = FhirCodeableConcept((code), ((reaction.get('substance').coding[0].display if reaction.get(
                            'substance').coding else None) if reaction.get('substance') else None), allergies_obj.fhir_allergy_idn, 'allergy', 'substance')
                        request.db.add(substance_obj)
                        request.db.flush()
                else:
                    substance_obj = FhirCodeableConcept(DictToObject({}), ((reaction.get('substance').coding[0].display if reaction.get(
                        'substance').coding else None) if reaction.get('substance') else None), allergies_obj.fhir_allergy_idn, 'allergy', 'substance')
                    request.db.add(substance_obj)
                    request.db.flush()
        ...
        for key, val in param_obj.items():
            if param_obj[key] != None:
                ...
                if obj_model['model'] in ('Reference', 'FhirCodeableConcept'):
                    if key == 'route':
                        attribute_value[obj_model['model']].append(
                            'exposureRoute')
                    else:
