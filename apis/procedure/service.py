from db.model.procedure import (FhirProcedure, CodeYn, FhirCodeableConcept, Reference, ProcedurePerformer,
...
def insert_procedure(request):
		...
		if type(procedure[key]).__name__ == 'list':
			for val in procedure[key]:
				if type(val).__name__ == 'Reference':
					ref_obj = Reference(val,procedure_obj.fhir_proc_idn,'procedure',key)
					request.db.add(ref_obj)
					
				if type(val).__name__ == 'CodeableConcept':
					if val.coding != None:
						for code in val.coding:
							Codeeable_obj = FhirCodeableConcept((code),val.text,procedure_obj.fhir_proc_idn,'procedure',key)
				...
				if type(val).__name__ == 'Annotation':
					insert_to_annotation(val,procedure_obj.fhir_proc_idn,'procedure',key,request.db)

				if type(val).__name__ == 'Identifier':
					insert_to_identifier(val,procedure_obj.fhir_proc_idn,'procedure',key,request.db)

		if type(procedure[key]).__name__ == 'Reference':
			ref_obj = Reference(procedure[key],procedure_obj.fhir_proc_idn,'procedure',key)
			request.db.add(ref_obj)

		if type(procedure[key]).__name__ == 'CodeableConcept':
			if procedure[key].coding != None:
				for code in procedure[key].coding:
					Codeeable_obj = FhirCodeableConcept((code),procedure[key].text,procedure_obj.fhir_proc_idn,'procedure',key)
					request.db.add(Codeeable_obj)
			else:
	...
	if procedure.performer != None:
		for performer in procedure.performer:
			actor_obj = Reference(performer.get('actor'),procedure_obj.fhir_proc_idn,'procedure','performer')
			request.db.add(actor_obj)
			request.db.flush()

			if performer.get('onBehalfOf') != None:
				onBehalfOf_obj = Reference(performer.get('onBehalfOf'),procedure_obj.fhir_proc_idn,'procedure','performer')
			...
			if performer.get('role') != None:
				if performer.get('role').coding != None:
					for code in performer.get('role').coding:
						role_obj = FhirCodeableConcept((code),performer.get('role').text,procedure_obj.fhir_proc_idn,'procedure','performer')
						request.db.add(role_obj)
						request.db.flush()
						performer_obj = ProcedurePerformer(procedure_obj.fhir_proc_idn,role_obj.codeable_concept_idn,actor_obj.reference_idn,
															onBehalfOf_idn)
				...
				performer_obj = ProcedurePerformer(procedure_obj.fhir_proc_idn,None,actor_obj.reference_idn,onBehalfOf_idn)
				request.db.add(performer_obj)
	manipulated_obj = None
	action_obj = None
	if procedure.focalDevice !=None:
		for focalDevice in procedure.focalDevice:
			manipulated_obj = Reference(focalDevice.get('manipulated'),procedure_obj.fhir_proc_idn,'procedure','focalDevice')
			...
			if focalDevice.get('action') != None:
				if focalDevice.get('action').coding != None:
					for code in focalDevice.get('action').coding:
						action_obj = FhirCodeableConcept((code),focalDevice.get('action').text,procedure_obj.fhir_proc_idn,'procedure','focalDevice')
						request.db.add(action_obj)
						request.db.flush()
						focalDevice_obj = ProcedureFocaldevice(procedure_obj.fhir_proc_idn,action_obj.codeable_concept_idn,manipulated_obj.reference_idn)
				...
				focalDevice_obj = ProcedureFocaldevice(procedure_obj.fhir_proc_idn,None,manipulated_obj.reference_idn)
				request.db.add(focalDevice_obj)
